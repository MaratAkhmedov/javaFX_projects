...
public class FXMLSettingsViewController implements Initializable {
    ...
    public int nEjs = 0;
    @FXML
    private Button compDur;
    @FXML
    private HBox durTotal;
    @FXML
    private Label durSeg;
    ...
    public int getNEjs() {
        nEjs = numEjercicios.getValue();
        return nEjs;
    }
    
    @FXML
    private void getTotalDuration(ActionEvent event) {
        durTotal.setVisible(true);
        
        int nCirc = numCircuitos.getValue();
        int descCirc = descansoEntreCircuitos.getValue();
        int descEj = descansoEntreEjercicios.getValue();
        int durEj = durEjercicio.getValue();
        int numEj = numEjercicios.getValue();      
        
        int tEjercicio = durEj*numEj*nCirc;
        int tDescanso = (descEj*((numEj - 1))*nCirc) + (descCirc*(nCirc - 1));
        
        int tTotalSeg = tEjercicio + tDescanso;
        durSeg.setText(String.valueOf(tTotalSeg));
